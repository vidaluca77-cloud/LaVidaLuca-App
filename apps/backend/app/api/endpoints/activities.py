...
def get_activities(
    skip: int = Query(
        0, ge=0, description="Number of activities to skip (for pagination)"
    ),
    limit: int = Query(
        100, ge=1, le=100, description="Maximum number of activities to return"
    ),
...
@router.get(
    "/{activity_id}", response_model=ActivitySchema, summary="Get activity by ID"
)
