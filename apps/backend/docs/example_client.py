...
class LaVidaLucaAPIClient:
    ...
    def get_contact_types(self) -> list:
        ...
        response = self.session.get(
            f"{self.base_url}/api/v1/contacts/types", headers=self._get_headers()
        )
        response.raise_for_status()
        return response.json()
    ...
    def get_suggestions(self) -> list:
        ...
        response = self.session.get(
            f"{self.base_url}/api/v1/suggestions/", headers=self._get_headers()
        )
        response.raise_for_status()
        return response.json()
...
def example_usage():
    ...
    try:
        ...
        print(
            f"✅ Available categories: {', '.join(categories) if categories else 'None yet'}"
        )
