/**
 * Custom hook for activity matching logic
 */
import { useMemo } from 'react';
import { Activity, UserProfile, Suggestion } from '@/types';

/**
 * Hook for calculating activity suggestions based on user profile
 * @param activities Array of available activities
 * @param profile User profile data
 * @returns Array of activity suggestions with scores
 */
  ...
  return useMemo(() => {
    ...
    const suggestions = activities.map(activity => {
      ...
      if (commonSkills.length > 0) {
        ...
        reasons.push(
          `Compétences correspondantes : ${commonSkills.join(', ')}`
        );
      }
      ...
      if (profile.preferences.includes(activity.category)) {
        ...
        reasons.push(
          `Catégorie préférée : ${categoryNames[activity.category]}`
        );
      }
