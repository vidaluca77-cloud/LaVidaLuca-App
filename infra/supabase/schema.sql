-- LaVida Luca Database Schema
-- PostgreSQL schema for Supabase
-- Created for La Vida Luca App - Agricultural education platform
...
-- =============================================
-- Activities Table (Catalogue)
-- =============================================
CREATE TABLE IF NOT EXISTS activities (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    activity_id VARCHAR(10) UNIQUE NOT NULL, -- Frontend ID (1, 2, 3...)
...
-- =============================================
-- Activity Registrations (Participations)
-- =============================================
...
-- Activities table policies (public read)
CREATE POLICY "Anyone can view active activities" ON activities
    FOR SELECT USING (is_active = true);

-- Only authenticated users can register and admin can manage
...
-- User profile with registration count
...
-- Activity summary with registration stats
...
COMMENT ON TABLE activities IS 'Catalogue of 30 educational agricultural activities';
COMMENT ON TABLE activity_registrations IS 'User registrations and participation tracking';

COMMENT ON COLUMN activities.activity_id IS 'Original frontend ID for compatibility';
COMMENT ON COLUMN activities.category IS 'agri=Agriculture, transfo=Transformation, artisanat=Artisanat, nature=Nature/Environment, social=Social/Animation';
COMMENT ON COLUMN activities.safety_level IS '1=Safe, 2=Moderate risk, 3=High supervision required';
COMMENT ON COLUMN activity_registrations.status IS 'pending=Awaiting confirmation, confirmed=Scheduled, completed=Finished, cancelled=Cancelled';